#!/usr/bin/env python3
"""
Lightweight Negative Binomial (NB2) anomaly detection for WEEKLY counts.
No C++ compiler required. Uses statsmodels.
"""
import argparse, os
import numpy as np
import pandas as pd
...
def predict_pi(res, X, level=0.99):
    # Separate alpha if present as last param
    params = res.params
...
def main():
    ...
    for _, pr in pairs.iterrows():
        ...
        for seg_start, seg_end in zip(seg_starts, seg_ends):
            ...
            gP, XP, yPseg = build_design(seg, args.fourier_K)
            try:
                resP = fit_nb2(yPseg, XP)
                muP, loP, hiP = predict_pi(resP, XP, args.pi_level)
                aic = resP.aic
        ...
        for key in keys:
            ...
            for seg_start, seg_end in zip(seg_starts_k, seg_ends_k):
                ...
                gX, X, yseg = build_design(seg, args.fourier_K)
                try:
                    res = fit_nb2(yseg, X)
                    mu, lo, hi = predict_pi(res, X, args.pi_level)
                    aic = res.aic
                except Exception as e:
